...
const MagicButton = ({
  ...
  return (
    <button className="relative inline-flex w-full h-12
    ...
    focus:outline-none md:w-60 md:mt-10" onClick={handleClick}>
        ...
        items-center justify-center rounded-lg bg-slate-950 px-7
        text-sm font-medium text-white backdrop-blur-3xl gap-2
        ${otherClasses}`}>
            {position === 'left' && icon}
            {title}
            {position === 'right' && icon}
